...
const RecipePopUp = ({enabled,meal,setEnablePopUp,setMealPopUp}) => {
    return(
        enabled && meal ? (
            <div className="recipePopUp">
                <div className='recipePopUp-inner'>
                    <iframe src={meal.strYoutube.replace('watch?v=','embed\/')} frameBorder="0"></iframe>
                    <div className="recipePopUp-inner-desc">
                        <p><span>Name : </span>{meal.strMeal}</p>
                        <p><span>Categorie : </span>{meal.strCategory}</p>
                        <p><span>Area : </span>{meal.strArea}</p>
                        <p><span>Instructions : </span>{meal.strInstructions}</p>
                    </div>
                    <div className='recipePopUp-back-cont'>
                        <button className='recipePopUp-back' onClick={()=>{
